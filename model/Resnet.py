...
class BasicBlock(nn.Module):  # ResNet模型中基本块定义
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.downsample = downsample  # 下采样，调整特征图通道数
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2)  # 残差块
        self.avgpool = nn.AvgPool2d(7, stride=1)
        self.fc = nn.Linear(512 * block.expansion, num_classes)

        self.load_state_dict(model_zoo.load_url(model_urls['resnet50']))  # 加载预训练的 ResNet-50 模型的权重参数
        ...
        self.fc = nn.Linear(512 * block.expansion, 4)  # 输出的类别数改为 4
        ...
        self.counting = nn.Linear(512 * block.expansion, 65)  # 输出的类别数改为 65
        ...
        def set_bn_fix(m):  # 将 Batch Normalization 层的参数设置为不可训练
            classname = m.__class__.__name__
            if classname.find('BatchNorm') != -1:
                for p in m.parameters(): p.requires_grad=False

        self.bn1.apply(set_bn_fix)  # 参数固定，不参与训练过程
        self.layer1.apply(set_bn_fix)
        self.layer2.apply(set_bn_fix)
        self.layer3.apply(set_bn_fix)
        self.layer4.apply(set_bn_fix)  # 用来冻结预训练模型的部分层，只训练模型的特定部分
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:  # 输入通道数是否与输出通道数一致
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample))  # 添加包含下采样的块
    ...
    def forward(self, x, x_new):
        ...
        cls = self.fc(x)
        cou = self.counting(x)  # 通过全连接层将特征向量映射到分类结果 cls 和计数结果 cou

        cls = F.softmax(cls) + 1e-4
        cou = F.softmax(cou) + 1e-4  # 避免除零错误
