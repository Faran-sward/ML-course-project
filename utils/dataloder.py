...
def load_data(config_dict, i):
    dset_train = DatasetProcessing(config_dict["data_path"], config_dict["train_files"][i], config_dict["num_classes"], 
            ...
            transforms.RandomRotation(degrees=config_dict["image_rotation"]),  # 随机旋转图像，角度的范围为 -20 到 +20 度之间
            config_dict["normalize"],  # 给定的均值和标准差进行归一化
        ]))

    dset_test = DatasetProcessing(config_dict["data_path"], config_dict["test_files"][i], config_dict["num_classes"], 
                                                     transform=transforms.Compose([
            transforms.Resize((config_dict["train_size"], config_dict["train_size"])),
            transforms.ToTensor(),
